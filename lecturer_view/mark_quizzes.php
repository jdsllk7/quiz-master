...
<body style="width:100%;min-height:500px;height:100%;">
	...
	<header class="w3-container w3-card-4 w3-theme w3-top" style="padding:15px; text-align:center;">
		...
		<h3 style="display:inline;">Quizzes</h3>
	</header>

	<form class="w3-container w3-center w3-large" style="margin-top:80px" method="GET" action="db/student_list_db.php">
		
		
		<?php
			
			$sql = "SELECT DISTINCT quiz_title FROM students";
			...
			if (mysqli_num_rows($result) > 0) {
				...
				while($row = mysqli_fetch_assoc($result)) {
					...
					echo'<tr>
							<li>'.$row["quiz_title"].'<button type="submit" name="quiz_to_mark" value="'.$row["quiz_title"].'" class="w3-btn view w3-tiny w3-green">Select student</button></li>
						</tr>';
